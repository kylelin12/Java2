...
public class BST < E extends Comparable < E >> implements Iterable < E > {
    ...
    */
	...
	private TreeNode < E > remove(TreeNode < E > rt, E val) {
		...
		// val is at the current root, rt.
		if (val.equals(rt.getValue())) {
			// no right subtree
			if (rt.getRight() == null) {
				// find the maximum value on the left subtree.
				E max = maxValue(rt.getLeft());
				// transfer the max value into rt
				rt.setValue(max);
				// remove max from the left subtree
			...
			// right subtree exits
				...
				// transfer the min value into rt
				rt.setValue(min);
		...
		else if (val.compareTo(rt.getValue()) < 0) rt.setLeft(remove(rt.getLeft(), val));
		else // travel down the right subtree
		rt.setRight(remove(rt.getRight(), val));
